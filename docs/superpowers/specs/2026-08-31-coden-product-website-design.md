...
文档内容在 `website/src/content/docs/{zh,en}/` 独立维护，不从根 README 自动生成。本次实现只建立文档基础设施、完整导航骨架、章节占位页和相应测试，不编写完整使用文档。后续填充正文时应以当前实现、测试和 README 为事实来源，重新组织内容，而不是逐段复制；命令、字段名和安全警告在两种语言间必须一致。
...
- 建立上述全部一级与二级导航项对应的中英文页面文件；
- 每个页面只包含标题、简短的范围说明和统一的未完成状态提示，不展开教程、配置细节、示例或参考正文；
- 中英文页面使用一致的稳定页面标识，确保路由、侧边栏、语言切换和搜索索引结构可验证；
- 建立用于检查页面映射、导航完整性、语言隔离、内部链接和静态构建结果的测试文件；
- 完整文档正文明确留给后续独立任务，不属于本次实现或验收范围。
...
3. 文档区建立快速入门、核心概念、界面、配置、Skills、插件、Agent Hooks、进阶和排错的完整中英文导航与占位页，但不要求实现完整正文。
4. 两种语言的占位页拥有稳定的一一映射、独立搜索索引和正确的语言切换，并有测试验证文档骨架完整性。
